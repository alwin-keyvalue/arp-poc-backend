...
Sanitize: title, summary, reason. Preserve: assignee, watchers, status, priority, all date fields, new_assignee."""
...
- status: appropriate initial status (to_do, done, dropped)
...
Extract updated_status and updated_due_date when present.""",

    IntentType.DATE_UPDATE: """## Date Update Extraction Rules

Extract updated_due_date when present.""",

    IntentType.REASSIGNMENT: """## Reassignment Extraction Rules

Extract new_assignee from the email.""",
