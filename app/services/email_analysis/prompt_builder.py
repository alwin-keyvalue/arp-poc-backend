...
def _format_email_context(llm_ctx: LLMEmailContext) -> str:
    ...
    if llm_ctx.thread_history:
        parts.append(f"Thread history (context only):\n{llm_ctx.thread_history}")
    return "\n\n".join(parts)
...
_FIELD_RULES = """## Rules
- Ignore email signatures. Never use Signature name as assignee.
- Extract only from Current message. Thread history is context only.
- assignee/watchers: @mentions without "@"; null if absent in Current message.
- priority (default P2): P0 = urgent/ASAP/immediately/critical; P1 = important/soon/today; P2 = no urgency stated.
- due_date: ISO 8601 with timezone; resolve relative dates from Email date (EOD→17:00morning 09:00, afternoon 14:00, evening 18:00, EOD 17:00)morning 09:00, afternoon 14:00, evening 18:00, EOD 17:00); null if none stated in current message. Never put deadlines in title/summary.
- title/summary: describe the work only (no assignee, no timing)."""

_EXTRACTION_RULES: dict[IntentType, str] = {
    IntentType.NEW_TASK: """## Extract
- title: short, action-oriented
- summary: work description (imperative), max 2 sentences
- status: to_do unless clearly otherwise
- Apply rules above.""",

    IntentType.TASK_UPDATE: """## Extract changed fields only (null for unchanged)
- due_date, assignee, watchers, status, title, summary, priority — only when explicitly changed in the current message.
...
_SANITIZATION_RULES = """## Sanitize title/summary for confidential/PII content; preserve assignee, watchers, status, priority, dates."""


def intent_prompt(llm_ctx: LLMEmailContext) -> tuple[str, str]:
    intent_values = ", ".join(f'"{i.value}"' for i in IntentType)
    system = f"""You are an email intent classifier for a task management system. Pick one: {intent_values}.
